...
function validateContentSections(page: SeoPageContent, where: string) {
  ...
  page.contentSections.forEach((block, index) => {
    ...
    if (block.cards !== undefined) assertTextItems(block.cards, `${at}.cards`);
    ...
    if (!block.body && !block.bullets && !block.cards && !block.table) {
      throw new Error(`${at} must include body, bullets, cards, or a table`);
    }
  });
}
