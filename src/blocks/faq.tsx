...
export async function FAQ() {
  ...
  return (
    <section id="faq" className="bg-brand-soft px-5 py-24 text-brand-ink sm:py-32">
        ...
        <Accordion className="overflow-hidden rounded-sm bg-brand-panel shadow-sm" defaultValue={["rights"]}>
          {FAQ_KEYS.map((key) => (
            <AccordionItem key={key} value={key} className="border-brand-line">
              <AccordionTrigger className="cursor-pointer px-5 py-6 text-left text-base font-black text-brand-muted hover:no-underline">
                {t(`faq.${key}.question`)}
              </AccordionTrigger>
              <AccordionContent className="px-5 pb-6 text-base font-semibold leading-8 text-brand-muted">
                {t(`faq.${key}.answer`)}
              </AccordionContent>
            </AccordionItem>
          ))}
